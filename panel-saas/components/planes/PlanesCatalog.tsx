...
const PLAN_SMS: Record<string, number> = { Básico: 200, Pro: 500, Premium: 1500 };
const PLAN_PROS: Record<string, number> = { Básico: 1, Pro: 3, Premium: 0 };
  ...
  return (
    <>
      <div className="grid gap-3 lg:grid-cols-3">
        {PLAN_CARDS.map(p => (
          <div key={p.name} className={`rounded-card bg-white p-5 ${p.featured ? 'ring-2 ring-ink' : 'border border-transparent'}`}>
            <div className="flex items-start justify-between gap-2">
              ...
              <ModalTrigger
                kind="plan"
                data={{ name: p.name, price: p.price, sms: PLAN_SMS[p.name], pros: PLAN_PROS[p.name] }}
                className="rounded-pill border border-line px-3 py-1.5 text-[12px] font-semibold"
              >
                Editar
              </ModalTrigger>
            </div>
      ...
      <div className="rounded-card bg-white p-5">
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          ...
          <ModalTrigger kind="bonus" className="h-10 rounded-pill bg-ink px-4 text-[13px] font-semibold text-white">
            + Nuevo bono
          </ModalTrigger>
        </div>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <div className="space-y-3 rounded-card bg-white p-5">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <ModalTrigger kind="referral" className="text-[13px] font-semibold text-brand-pink">
              Editar reglas
            </ModalTrigger>
          </div>
