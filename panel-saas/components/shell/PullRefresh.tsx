...
function skipTarget(from: EventTarget | null) {
  ...
  return !!from.closest('input, textarea, select, [contenteditable="true"], [role="dialog"], [data-no-pull]');
}

/** En el móvil, deslizar hacia abajo recarga la pantalla de Ops. */
export default function PullRefresh({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const onMove = (e: TouchEvent) => {
      ...
      e.preventDefault();
      pull.current = Math.min(MAX, dy * 0.42);
    ...
    const finish = () => {
      ...
      paint(48, true);
      setBusy(true);
      try { navigator.vibrate?.(12); } catch { /* el navegador no vibra */ }
  ...
  return (
    <div ref={rootRef} className="relative flex h-0 min-h-0 flex-1 flex-col overflow-hidden lg:h-auto lg:flex-none lg:overflow-visible">
      <div
        ref={knobRef}
        aria-hidden
        className="pointer-events-none absolute inset-x-0 top-0 z-20 flex justify-center opacity-0 lg:hidden"
      >
        <Loader2
          size={22}
          strokeWidth={2.4}
          className={`text-brand-pink ${busy ? 'motion-safe:animate-spin' : ''}`}
        />
      </div>
      <div ref={shiftRef} className="flex h-0 min-h-0 flex-1 flex-col lg:h-auto lg:flex-none">
        {children}
      </div>
